                            ...
                            <div class="col-md-12">
                                <div class="card-box pb-10">
                                   <form role="form" enctype="multipart/form-data" method="post" action="">
                                        <div>
                                            <h3 class="p-2">Send Request </h3>
                                        </div>
                                        <div class="row m-2">
                                            <div class="col-md-8">
                                                <div class="form-group m-2">
                                                    <label>Email</label>
                                                    <input class="form-control" type="email" value="<?php if(!empty($tax_data['username'])){echo $tax_data['username'];}else{echo '';}?>" name="username" placeholder="Email" />
                                                </div>
                                            </div>
...
$(document).ready(function(){
  ...
  $(".dropdown-item").click(function(e){
    var val = confirm("Are you sure, you want to delete sendrequest ?");
    if (val == true){
    ...
    if(val){
      $.ajax({
        ...
        data:{table:'sendrequest',id:id,status:3,colwhr:'id',whrstatuscol:'status',action:"Delete"},
        dataType:'json',
      ...
      window.location.reload();
        ...
        if(val){
            $.ajax({
              ...
              data:{tablename:'sendrequest',id:id,status:0,whrcol:'id',whrstatuscol:'status',action:"Deactive"},
              dataType:'json',
              success: function(response) {
                if (response.status == 1){
                  $.notify(response.msg, "success");
  ...
  $(".useractive").click(function(e){
    var val = confirm("Are you, sure you want to activate send request ?");
    if (val == true){
      window.location.reload();
    }
    ...
    if(val){
      $.ajax({
        type: "POST",
        url: "<?php echo base_url();?>admin/welcome/change_status",
        data:{tablename:'sendrequest',id:id,status:1,whrcol:'id',whrstatuscol:'status',action:"Active"},
        dataType:'json',
        success: function(response) {
          if (response.status == 1){
            $.notify(response.msg, "success");
