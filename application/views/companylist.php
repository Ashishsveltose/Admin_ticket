                ...
                <div class="row align-items-center">
                    <div class="col-md-12">
                        <div class="page-header-title">
                            <h5 class="m-b-10">Company List</h5>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="col-xl-12">
                    <div class="card">
                        <div class="card-header">
                            <div class="row">
                                <div class="col-md-4">
                                    <button type="button" class="btn btn-primary" onclick="business()">Create Company</button>
                                </div>
    ...
    foreach ($taxlist as $getdata) { 
       ?>
                                        <tr>
                                            ...
                                            <td><?php echo (!empty($getdata['company_name']) ? $getdata['company_name'] : 'none'); ?></td>
                                            <td>
                                                
<a   style="margin-left:10px;" href="<?php echo base_url() ?>welcome/addcompany/<?php echo $getdata['id']; ?>" title="Edit">
                        <svg xmlns="http://www.w3.org/2000/svg"   width="25" height="25" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
  <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
  <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5z"/>
</svg>
                        </a>
                                                 <a href="javascript:void(0)" href-data="<?php echo  $getdata['id']; ?>" class="dropdown-itemm"   title="Delete">
                                                 <svg xmlns="http://www.w3.org/2000/svg" width="25" color="red" height="25" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
...
<script>
    function business() {
        ...
        window.location.href = 'https://supporthub.xytr.org/ticket_admin/addcompany';
    }
</script>
...
$(document).ready(function(){
  ...
  $(".dropdown-itemm").click(function(e){
    var val = confirm("Are you sure, you want to delete  business area ?");
    if (val == true){
    ...
    if(val){
      $.ajax({
        ...
        data:{table:'companylist',id:id,status:3,colwhr:'id',whrstatuscol:'status',action:"Delete"},
        dataType:'json',
     ...
     setTimeout(function(){location.reload()},1000);
        ...
        if(val){
            $.ajax({
              ...
              data:{tablename:'companylist',id:id,status:0,whrcol:'id',whrstatuscol:'status',action:"Deactive"},
              dataType:'json',
              success: function(response) {
                if (response.status == 1){
                  $.notify(response.msg, "success");
  ...
  $(".useractive").click(function(e){
    var val = confirm("Are you, sure you want to activate companylist?");
    if (val == true){
       setTimeout(function(){location.reload()},1000);
    }
    ...
    if(val){
      $.ajax({
        type: "POST",
        url: "<?php echo base_url();?>welcome/change_status",
        data:{tablename:'companylist',id:id,status:1,whrcol:'id',whrstatuscol:'status',action:"Active"},
        dataType:'json',
        success: function(response) {
          if (response.status == 1){
            $.notify(response.msg, "success");
