...
<th>Status</th>
   ...
   // $category1 = $this->Common_model->getSingleRecordById('article_user',array('id'=> $getdata['user_id']));
                  ...
                  ?>
                    ...
                    <td><?php  echo (!empty($getdata['tittle'])?$getdata['tittle']:'none'); ?></td>
...
$(document).ready(function(){
  ...
  $(".dropdown-itemm").click(function(e){
    var val = confirm("Are you sure, you want to delete article pages?");
    if (val == true){
    ...
    if(val){
      $.ajax({
        ...
        data:{table:'article_pages',id:id,status:3,colwhr:'id',whrstatuscol:'status',action:"Delete"},
        dataType:'json',
  ...
  $(".deactive").click(function(e){
        var val = confirm("Are you sure, you want to articles pages  ?");
        if (val == true){
     setTimeout(function(){location.reload()},1000);
    }
        ...
        if(val){
            $.ajax({
              ...
              data:{tablename:'article_pages',id:id,status:0,whrcol:'id',whrstatuscol:'status',action:"Deactive"},
              dataType:'json',
              success: function(response) {
                if (response.status == 1){
                  $.notify(response.msg, "success");
  ...
  $(".useractive").click(function(e){
    var val = confirm("Are you, sure you want to activate article  article pages?");
    if (val == true){
       setTimeout(function(){location.reload()},1000);
    }
    ...
    if(val){
      $.ajax({
        type: "POST",
        url: "<?php echo base_url();?>welcome/change_status",
        data:{tablename:'article_pages',id:id,status:1,whrcol:'id',whrstatuscol:'status',action:"Active"},
        dataType:'json',
        success: function(response) {
          if (response.status == 1){
            $.notify(response.msg, "success");
