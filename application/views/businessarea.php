                ...
                <div class="row align-items-center">
                    <div class="col-md-12">
                        <div class="page-header-title">
                            <h5 class="m-b-10">Business Area List</h5>
                        </div>
                    </div>
                </div>
            </div>
    ...
    foreach ($taxlist as $getdata) { 
       ?>
                                        <tr>
                                            ...
                                            <td><?php echo (!empty($getdata['area_name']) ? $getdata['area_name'] : 'none'); ?></td>
                                            <td><?php echo (!empty($getdata['note']) ? $getdata['note'] : 'none'); ?></td>
                                            <td>
                                                
<a   style="margin-left:10px;" href="<?php echo base_url() ?>welcome/createbusiness/<?php echo $getdata['id']; ?>" title="Edit">
...
<script>
    function business() {
        ...
        window.location.href = 'https://supporthub.xytr.org/ticket_admin/createbusiness';
    }
</script>
  ...
  $(".dropdown-itemm").click(function(e){
    ...
    if(val){
      $.ajax({
        ...
        data:{table:'business_area',id:id,status:3,colwhr:'id',whrstatuscol:'status',action:"Delete"},
        dataType:'json',
     ...
     setTimeout(function(){location.reload()},1000);
        ...
        if(val){
            $.ajax({
              ...
              data:{tablename:'userlist',id:id,status:0,whrcol:'id',whrstatuscol:'status',action:"Deactive"},
              dataType:'json',
              success: function(response) {
                if (response.status == 1){
                  $.notify(response.msg, "success");
  ...
  $(".useractive").click(function(e){
    ...
    if(val){
      $.ajax({
        type: "POST",
        url: "<?php echo base_url();?>welcome/change_status",
        data:{tablename:'business_area',id:id,status:1,whrcol:'id',whrstatuscol:'status',action:"Active"},
        dataType:'json',
        success: function(response) {
          if (response.status == 1){
            $.notify(response.msg, "success");
