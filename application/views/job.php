            ...
            <div class="pd-ltr-20 xs-pd-20-10">
                <div class="min-height-200px">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                              <div class="pd-20 float-right">
                                   <a href="https://edumatelive.in/newadmin/addjob" class="dropdown-toggle no-arrow">
                  ...
                  ?>
                    ...
                    <td><?php  echo (!empty($getdata['job_offer_id'])?$getdata['job_offer_id']:'none'); ?></td>
                      <td><?php  echo (!empty($getdata['title'])?$getdata['title']:'none'); ?></td>
                     
                      <td><?php  echo (!empty($getdata['buyer_name'])?$getdata['buyer_name']:'none'); ?></td>
...
$(document).ready(function(){
  ...
  $(".dropdown-item").click(function(e){
    var val = confirm("Are you sure, you want to delete job  ?");
    if (val == true){
    ...
    if(val){
      $.ajax({
        ...
        data:{table:'job',id:id,status:3,colwhr:'id',whrstatuscol:'status',action:"Delete"},
        dataType:'json',
  ...
  $(".deactive").click(function(e){
        var val = confirm("Are you sure, you want to deactivate job ?");
        if (val == true){
      window.location.reload();
    }
        ...
        if(val){
            $.ajax({
              ...
              data:{tablename:'job',id:id,status:0,whrcol:'id',whrstatuscol:'status',action:"Deactive"},
              dataType:'json',
              success: function(response) {
                if (response.status == 1){
                  $.notify(response.msg, "success");
  ...
  $(".useractive").click(function(e){
    var val = confirm("Are you, sure you want to activate job ?");
    if (val == true){
      window.location.reload();
    }
    ...
    if(val){
      $.ajax({
        type: "POST",
        url: "<?php echo base_url();?>admin/welcome/change_status",
        data:{tablename:'job',id:id,status:1,whrcol:'id',whrstatuscol:'status',action:"Active"},
        dataType:'json',
        success: function(response) {
          if (response.status == 1){
            $.notify(response.msg, "success");
