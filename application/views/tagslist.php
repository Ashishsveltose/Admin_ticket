                            ...
                            <div class="col-md-12">
                              <!--<div class="col-6">-->
                                <div class="pd-20 float-right">
                                    <a href="<?php echo base_url();?>article_tags" class="dropdown-toggle no-arrow">
                                        <button type="button" class="btn btn-primary">Add Tags</button>
                                    </a> 
                              </div>
                              <!--   </div> -->
   ...
   // $category1 = $this->Common_model->getSingleRecordById('article_user',array('id'=> $getdata['user_id']));
                  ...
                  ?>
                    ...
                    <td><?php  echo (!empty($getdata['name'])?$getdata['name']:'none'); ?></td>
                                        <td><?php  echo (!empty($getdata['slug'])?$getdata['slug']:'none'); ?></td>
                        ...
                        <td>
                                                        ...
                                                        <div
                                                            ...
                                                            <a href="javascript:void(0)" href-data="<?php echo  $getdata['id']; ?>" class="dropdown-itemm" title="Delete"><i class="dw dw-delete-3" aria-hidden="true"></i>Delete</a>
                                                                                                                     <a style="margin-left:10px;"  href="<?php echo base_url() ?>welcome/addarticles/<?php echo  $getdata['id']; ?>" title="Edit"><img src="<?php echo base_url();?>home/assets/vendors/images/edit.png" alt="" class="dark-logo" style="width:20px;height:20px;"/>Edit</a>
...
$(document).ready(function(){
  ...
  $(".dropdown-itemm").click(function(e){
    var val = confirm("Are you sure, you want to delete article tags?");
    if (val == true){
    ...
    if(val){
      $.ajax({
        ...
        data:{table:'article_tags',id:id,status:3,colwhr:'id',whrstatuscol:'status',action:"Delete"},
        dataType:'json',
  ...
  $(".deactive").click(function(e){
        var val = confirm("Are you sure, you want to articles tags  ?");
        if (val == true){
     setTimeout(function(){location.reload()},1000);
    }
        ...
        if(val){
            $.ajax({
              ...
              data:{tablename:'article_tags',id:id,status:0,whrcol:'id',whrstatuscol:'status',action:"Deactive"},
              dataType:'json',
              success: function(response) {
                if (response.status == 1){
                  $.notify(response.msg, "success");
  ...
  $(".useractive").click(function(e){
    var val = confirm("Are you, sure you want to activate article  tags?");
    if (val == true){
       setTimeout(function(){location.reload()},1000);
    }
    ...
    if(val){
      $.ajax({
        type: "POST",
        url: "<?php echo base_url();?>welcome/change_status",
        data:{tablename:'article_tags',id:id,status:1,whrcol:'id',whrstatuscol:'status',action:"Active"},
        dataType:'json',
        success: function(response) {
          if (response.status == 1){
            $.notify(response.msg, "success");
