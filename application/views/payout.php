            ...
            <div class="pd-ltr-20 xs-pd-20-10">
                <div class="min-height-200px">
                    <div class="card-box mb-30">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                              ...
                              <!--      <a href="https://edumatelive.in/newadmin/addrefer" class="dropdown-toggle no-arrow">
                  ...
                  ?>
                    ...
                    <td><?php  echo (!empty($getdata['payment_gateway'])?$getdata['payment_gateway']:'none'); ?></td>
                      <td><?php  echo (!empty($getdata['amount'])?$getdata['amount']:'none'); ?></td>
                      <td><?php  echo (!empty($getdata['request_date'])?$getdata['request_date']:'none'); ?></td>
                      <td>Active</td>
                        <td>
  ...
  $(".dropdown-item").click(function(e){
    ...
    if(val){
      $.ajax({
        ...
        data:{table:'payout',id:id,status:3,colwhr:'id',whrstatuscol:'status',action:"Delete"},
        dataType:'json',
      ...
      window.location.reload();
        ...
        if(val){
            $.ajax({
              ...
              data:{tablename:'payout',id:id,status:0,whrcol:'id',whrstatuscol:'status',action:"Deactive"},
              dataType:'json',
              success: function(response) {
                if (response.status == 1){
                  $.notify(response.msg, "success");
  ...
  $(".useractive").click(function(e){
    ...
    if(val){
      $.ajax({
        type: "POST",
        url: "<?php echo base_url();?>admin/welcome/change_status",
        data:{tablename:'payout',id:id,status:1,whrcol:'id',whrstatuscol:'status',action:"Active"},
        dataType:'json',
        success: function(response) {
          if (response.status == 1){
            $.notify(response.msg, "success");
